...
int main() {
	...
	meminit(fp, allSize);
	testBlock* a1 = fp;
	uint32_t* trial = (uint32_t*)memalloc(sizeof(uint32_t*));
	 a1 = fp;
	printf("%d\n", *(trial-4));
	uint32_t* trial2 = (uint32_t*)memalloc(sizeof(uint32_t*));
	uint32_t* trial3 = (uint32_t*)memalloc(sizeof(uint32_t*));
	//printf("%p, %p\n", trial, fp  );
	//printf("%d\n",  trial - fp);
	//printf("%d\n", trial2-trial);
	...
	memfree(trial);
	a1 = fp;
	testBlock* a2 = (testBlock*)((char*)a1 + *BlockSizeHead(a1) + DESC_SIZE);
	memfree(trial2);
	uint32_t* elem = myList.desc;
	printf("%p\n", fp);
